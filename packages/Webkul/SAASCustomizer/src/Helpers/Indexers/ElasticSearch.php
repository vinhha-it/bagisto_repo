...
class ElasticSearch extends AbstractIndexer
{
    ...
    {
        $attributes = $this->attributeRepository->findWhereIn('code', ['visible_individually', 'status'])->pluck('id', 'code')->toArray();
        ...
        while (true) {
            $paginator = $this->productRepository
                ...
                ->join('product_attribute_values as visible_individually_pav', function ($join) use ($attributes) {
                    $join->on('products.id', '=', 'visible_individually_pav.product_id')
                        ->where('visible_individually_pav.attribute_id', $attributes['visible_individually'])
                        ->where('visible_individually_pav.boolean_value', 1);
                })
                ->join('product_attribute_values as status_pav', function ($join) use ($attributes) {
                    $join->on('products.id', '=', 'status_pav.product_id')
                        ->where('status_pav.attribute_id', $attributes['status'])
