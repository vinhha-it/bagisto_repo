<x-saas::layouts>
    <!-- Title of the page. -->
    <x-slot:title>
        @lang('saas::app.super.configuration.index.title')
    </x-slot:title>
        ...
        <v-configuration-search>
            <div class="relative flex w-[525px] max-w-[525px] items-center max-lg:w-[400px] ltr:ml-2.5 rtl:mr-2.5">
                ...
                <input 
                    ...
                    placeholder="@lang('saas::app.super.configuration.index.search')" 
                >
            </div>
        </v-configuration-search>
    </div>
    ...
    <div class="grid gap-y-8">
        ...
        @foreach (super_system_config()->getItems() as $itemKey => $item)
            <div>
                ...
                <div class="box-shadow max-1580:grid-cols-3 mt-2 grid grid-cols-4 flex-wrap justify-between gap-12 rounded bg-white p-4 max-xl:grid-cols-2 max-sm:grid-cols-1 dark:bg-gray-900">
                    <!-- Menus cards -->
                    @foreach ($item->getChildren() as $childKey =>  $child)
                        <a 
                            ...
                            href="{{ count($child->getChildren()) ? route('super.configuration.index', ($item->getKey().'/'.$childKey)) : '#' }}"
                        >
                            @if ($icon = $child->getIcon())
                                <img
                                    class="h-[60px] w-[60px] dark:mix-blend-exclusion dark:invert"
                                    src="{{ bagisto_asset('images/'.$icon, 'admin') }}"
                                >
                            @endif
                ...
                >
                    ...
                    <template v-else>
                        <div class="grid max-h-[400px] overflow-y-auto">
                            ...
                            >
                                @lang('saas::app.super.configuration.index.no-result-found')
                            </div>
                        </div>
                    </template>
                </div>
            ...
            app.component('v-configuration-search', {
                ...
                methods: {
                    search() {
                        ...
                        this.$axios.get("{{ route('super.configuration.search') }}", {
