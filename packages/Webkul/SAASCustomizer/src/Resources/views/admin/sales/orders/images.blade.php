<div class="flex gap-1.5">
    ...
    @foreach ($items as $item)
        ...
        <div class="relative">
            <div class="w-full h-[60px] max-w-[60px] max-h-[60px] relativerounded">
                @if ($imageCount > 0)
                    <img 
                        class="w-full h-full rounded" 
                        src="{{ $item->product->base_image_url }}"
                    >
                    <span class="absolute bottom-px ltr:left-px rtl:right-px text-xs font-bold text-white bg-darkPink rounded-full px-1.5">{{ $imageCount }}</span>
                @else
                    <div class="w-full h-[60px] max-w-[60px] max-h-[60px] relative border border-dashed border-gray-300 dark:border-gray-800 rounded dark:invert dark:mix-blend-exclusion">
                        ...
                        <p class="absolute w-full bottom-1.5 text-xs text-gray-400 text-center font-semibold"> 
